...
def test_alignment_ocs():
    ...
    occu_probs, vec = [], None
    for t_ind in range(int(t_end / dt) + 1):
        print('  t = {:6.2f} ps'.format(t_ind * dt), end='\r')
        ham = h0 - 0.5 * AUpol_x_Vm_to_invcm() * alpha * field[t_ind]
        vec = ham.U(dt * 1e-12, vec=vec, matvec_lib='scipy')
        if t_ind % 10 == 0:
            occu_probs.append(
               [ round(t_ind * dt, 1), 
                 *[ round(abs(vec[J][ham.symlist2[J][0]][0])**2, 4)
                    for J in J_list ] ]
            )
            #print('    result: ', occu_probs[-1],
            #      '    reference: ', ref_occu_probs[int(t_ind / 10)])
