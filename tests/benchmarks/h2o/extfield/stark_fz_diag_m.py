...
###################################################################
# Example of calculation of rovibrational energies and transition
# linestrengths for H2O molecule placed in static electric field.
# Apply static field along Z axis.
###################################################################

richmol_file = '../convert_rchm_to_h5/H2O_j0_j5.h5'
...
field_z = 10000 * 100 # 10 kV/cm in units V/m
...
for m in range(-Jmax, Jmax+1):
    ...
    # field-free states
    states = States(richmol_file, 'h0', [J for J in range(Jmax + 1)], emin=0, emax=10000, m_list=[m], verbose=True)
    ...
    try:
        ...
        fac = mu_au_to_Cm / (planck * c_vac) / 100.0 * 0.393456 # the last factor for dipole moment from Debye to au
        mu.mul(fac)
...
for m1 in range(-Jmax, Jmax+1):
    ...
    states1 = States(richmol_file, 'h0', [J for J in range(Jmax + 1)], emin=0, emax=10000, m_list=[m1])

    for m2 in range(-Jmax, Jmax+1):
        if abs(m1 - m2) > 1: continue # selection rules

        states2 = States(richmol_file, 'h0', [J for J in range(Jmax + 1)], emin=0, emax=10000, m_list=[m2])

        try:
            mu = Tensor(richmol_file, 'dipole', states1, states2)
        except NoCouplingError:
            continue
