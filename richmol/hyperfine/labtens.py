...
class LabTensor(CarTens):
    ...
    def __init__(self, h0, tens, thresh=None):
        ...
        self.cart = tens.cart
        try:
            self.cart_ind_ = self.cart_ind[self.rank]
        except KeyError:
            raise KeyError(
                ...
                for f2 in h0.Jlist2:
                    for sym2 in h0.symlist1[f2]:
                        ...
                        for omega in set([o for (o, s) in self.os]):
                            me = self._kPrim(q1, q2, tens, omega)
        ...
        # M-tensor
        cart_ind = [self.cart_ind_.index(cart) for cart in self.cart]
        for f1 in h0.Jlist1:
            for sym1 in h0.symlist1[f1]:
                q1 = h0.quanta_m1
                for f2 in h0.Jlist2:
    ...
    def _kPrim(quantaSpinJSym1, quantaSpinJSym2, tens, omega):
        hMat = []
    ...
    def _mPrim(quanta_m1, quanta_m2, omega):
        ...
        for i, m1 in enumerate(quanta_m1):
            ...
            for j, m2 in enumerate(quanta_m2):
                threej = py3nj.wigner3j([int(f1*2)]*n, [int(omega*2)]*n, [int(f2*2)]*n,
                                        [int(-m1*2)]*n, sigma*2, [int(m2*2)]*n)
